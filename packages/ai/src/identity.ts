...
function detectIdentityIntent(
    message: string,
): IdentityIntent | null {
    const text = normalize(message);

    if (
        /\b(what|whats|tell me)\b.*\b(name|called)\b/.test(
            text,
        ) ||
    ...
    // Creator
    if (/\b(who|how\what)\b.*\b(created|made|built|developed|configured|designed|set up|setup)\b/.test(
        text,
    ) ||
    /\b(creator|creator of|maker|developer|behind)\b/.test(
        text,
    ) ||
    ...
    if(
        /\b(who|whose)\b.*\b(owns|owner|belong)\b/.test(
            text,
        ) ||
